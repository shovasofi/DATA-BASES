...
CREATE INDEX Title_ASC_index
    ON Films (title ASC);
    
SHOW INDEX FROM Films;

--Indexed querys explained with EXPLAIN ANALYZE
...
-- -> Index lookup on Films using Title_ASC_index (title='The Boss Baby')  (cost=0.35 rows=1) (actual time=0.024..0.026 rows=1 loops=1)
...
-- -> Index lookup on Films using Title_ASC_index (title='Red Dragon')  (cost=0.35 rows=1) (actual time=0.021..0.023 rows=1 loops=1)
...
-- -> Sort: Films.title  (cost=0.35 rows=1) (actual time=0.035..0.035 rows=1 loops=1)
-- -> Index lookup on Films using FK_ProducertoFilm (producer_id='5')  (actual time=0.022..0.025 rows=1 loops=1)


--Notes: as you see, the cost and average execution time costs less with index than without it
